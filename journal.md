...
### 13. Esc キーで初期画面に戻る機能
- 入力画面から初期画面（Idle）に戻す手段がなかった。
- M5Cardputer のキーボードライブラリは Fn キーを修飾フラグ（`keys.fn`）として渡すだけで、キーコード自体を変えない仕様。
- `keys.fn && key == '`'`（Fn + バッククォート）の組み合わせを Esc として処理し、全状態から Idle に戻るよう実装。
- ヘルプ表示に `Esc:戻る` を追加。
